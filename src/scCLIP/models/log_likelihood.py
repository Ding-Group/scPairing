...
def _log_zigamma(x, alpha, beta, pi, eps=1e-8):
    x_hat = x.clone()
    x_hat = torch.clamp(x_hat, min=0.5)
    where_zero = (x < eps).float()
    where_nonzero = (x > eps).float()

    softplus_pi = F.softplus(-pi)
    gam = Gamma(alpha, beta)

    case_zero = -pi - softplus_pi
    case_non_zero = -softplus_pi + gam.log_prob(x_hat)

    mul_case_zero = case_zero * where_zero
    mul_case_non_zero = case_non_zero * where_nonzero
    res = mul_case_zero + mul_case_non_zero
    return res.sum(-1)
