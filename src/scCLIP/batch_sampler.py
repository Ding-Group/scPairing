...
# TODO: Absorb this class into the CellSampler
class TriCellSampler():
    ...
    def __init__(self,
        ...
        Args:
            adata1: an AnnData object storing the dataset of the first modality.
            adata2: an AnnData object storing the dataset of the second modality.
            batch_size: size of each sampled minibatch.
            counts_layer: AnnData layer corresponding to raw counts. If it is a singular str,
                ...
                the first will be applied to adata1, second applied to adata2
        ...
        Assumption that adata1 and adata2 have the same cells in the same order.
        """
        assert adata1.n_obs == adata2.n_obs == adata3.n_obs, "The two AnnData objects have a different number of observations"
        self.n_cells: int = adata1.n_obs
        self.batch_size: int = batch_size
        self.n_epochs: Union[int, float] = n_epochs
        self.require_counts: bool = require_counts
        ...
        if transformed_obsm is not None:
            if isinstance(transformed_obsm, str):
                transformed_obsm = [transformed_obsm, transformed_obsm, transformed_obsm]
            self.X_1_transformed = adata1.obsm[transformed_obsm[0]] if transformed_obsm[0] else adata1.X
            self.X_2_transformed = adata2.obsm[transformed_obsm[1]] if transformed_obsm[1] else adata2.X
            self.X_3_transformed = adata3.obsm[transformed_obsm[2]] if transformed_obsm[2] else adata3.X
        else:
            self.X_1_transformed = adata1.X
            self.X_2_transformed = adata2.X
            self.X_3_transformed = adata3.X
    ...
    def _high_batch_size(self) -> Iterator[Mapping[str, torch.Tensor]]:
        ...
        X_1_transformed = FloatTensor(self.X_1_transformed.todense() if isinstance(self.X_1_transformed, spmatrix) else self.X_1_transformed)
        X_2_transformed = FloatTensor(self.X_2_transformed.todense() if isinstance(self.X_2_transformed, spmatrix) else self.X_2_transformed)
        X_3_transformed = FloatTensor(self.X_3_transformed.todense() if isinstance(self.X_3_transformed, spmatrix) else self.X_3_transformed)
    ...
    def _low_batch_size(self) -> Iterator[Mapping[str, torch.Tensor]]:
        ...
        while count < self.n_epochs:
            ...
            library_size_1 = FloatTensor(self.library_size_1[batch])
            library_size_2 = FloatTensor(self.library_size_2[batch])
            library_size_3 = FloatTensor(self.library_size_3[batch])
            ...
            if not self.require_counts:
                ...
                cells_1 = FloatTensor(X_1.todense() if isinstance(X_1, spmatrix) else X_1)
                cells_2 = FloatTensor(X_2.todense() if isinstance(X_2, spmatrix) else X_2)
                cells_3 = FloatTensor(X_3.todense() if isinstance(X_3, spmatrix) else X_3)

            X_1_transformed = self.X_1_transformed[batch, :]
            X_2_transformed = self.X_2_transformed[batch, :]
            X_3_transformed = self.X_3_transformed[batch, :]

            X_1_transformed = FloatTensor(X_1_transformed.todense() if isinstance(X_1_transformed, spmatrix) else X_1_transformed)
            X_2_transformed = FloatTensor(X_2_transformed.todense() if isinstance(X_2_transformed, spmatrix) else X_2_transformed)
            X_3_transformed = FloatTensor(X_3_transformed.todense() if isinstance(X_3_transformed, spmatrix) else X_3_transformed)
